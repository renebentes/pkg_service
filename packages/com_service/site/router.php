...
function ServiceBuildRoute(&$query)
{
	...
	// Are we dealing with an service that is attached to a menu item?
	...
	if (isset($view) and ($view == 'category' or $view == 'service'))
	{
...
function ServiceParseRoute($segments)
{
	...
	$categories = JCategories::getInstance('Service')->get($id)->getChildren();
	$vars['catid'] = $id;
	$vars['id'] = $id;
	$found = 0;
	...
	foreach ($segments as $segment)
	{
		...
		if ($found == 0)
		{
			...
			{
				$db = JFactory::getDBO();
				$query = 'SELECT id FROM #__service WHERE catid = ' . $vars['catid'] . ' AND alias = ' . $db->Quote($segment);
				$db->setQuery($query);
				$nid = $db->loadResult();
			}
			else
